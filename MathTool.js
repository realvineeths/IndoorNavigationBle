...
// class MathTool {
//     // static  RSSI_TO_DISTANCE_A = 60;
//     // static RSSI_TO_DISTANCE_N = 3.3;
...
class MathTool {
    ...
    static PointVector2 = class {
      constructor(p1, p2) {
        this.p1 = new MathTool.Point(p1.x, p1.y);
        this.p2 = new MathTool.Point(p2.x, p2.y);
      }
    };
  
    static Circle = class {
      constructor(center, r) {
        this.center = new MathTool.Point(center.x, center.y);
        this.r = r;
      }
    };
    ...
    //   return (
    //     0.882909233 * Math.pow(rssi / -58, 4.57459326) + 0.045275821
    //   );
    // }
    ...
    static isTwoCircleIntersect(c1, c2) {
      return MathTool.getDistanceBetweenTwoPoint(c1.center, c2.center) < c1.r + c2.r;
    }
  
    static getIntersectionPointsOfTwoIntersectCircle(c1, c2) {
      let pointVector2 = new MathTool.PointVector2(new MathTool.Point(0, 0), new MathTool.Point(0, 0));
      if (c1.center.y === c2.center.y && c1.center.y === 0) {
        let ct1 =
          c1.center.x < c2.center.x
            ? new MathTool.Circle(new MathTool.Point(c1.center.x, c1.center.y), c1.r)
            : new MathTool.Circle(new MathTool.Point(c2.center.x, c2.center.y), c2.r);
        let ct2 =
          c1.center.x < c2.center.x
            ? new MathTool.Circle(new MathTool.Point(c2.center.x, c2.center.y), c2.r)
            : new MathTool.Circle(new MathTool.Point(c1.center.x, c1.center.y), c1.r);
        let l = MathTool.getDistanceBetweenTwoPoint(ct1.center, ct2.center);
        let cos = (ct1.r * ct1.r + l * l - ct2.r * ct2.r) / (2 * ct1.r * l);
        if (cos > 1) cos = 0;
      ...
      if ((c1.center.x === 0 && c2.center.y === 0) || (c2.center.x === 0 && c1.center.y === 0)) {
        ...
        let ct2 =
          c1.center.y === 0
            ...
            : new MathTool.Circle(new MathTool.Point(c2.center.x, c2.center.y), c2.r);
        let l = MathTool.getDistanceBetweenTwoPoint(ct1.center, ct2.center);
        let aCos = (ct1.r * ct1.r + l * l - ct2.r * ct2.r) / (2 * ct1.r * l);
        let aAngle = Math.acos(aCos);
        let aTan = ct2.center.x / ct1.center.y;
        let bAngle = Math.atan(aTan);
        pointVector2.p1.x = ct1.center.x + Math.sin(bAngle - aAngle);
        pointVector2.p1.y = ct1.center.y - Math.cos(bAngle - aAngle);
        pointVector2.p2.x = ct1.center.x + Math.sin(bAngle + aAngle);
        pointVector2.p2.y = ct1.center.y - Math.cos(bAngle - aAngle);
    ...
    static getCenterOfThreePoint(p1, p2, p3) {
      return new MathTool.Point((p1.x + p2.x + p3.x) / 3, (p1.y + p2.y + p3.y) / 3);
    }
}
